...
class ExportData(peak.OneShotBehaviour):
    ...
    def __init__(self, file_name: str, properties: list, interval: int = None, to_graph: bool = False, graph_name: str = '', graph_options: str = ''):
        super().__init__()
    ...
    async def run(self) -> None:
        ...
        if isinstance(self.agent, peak.SyncAgent):
            ...
            self.agent.add_behaviour(_ExportDataSync(self.file_name, self.properties, self.to_graph, self.graph_name, self.graph_options))
        else:
            logger.debug('synchronizer not detected')
            self.agent.add_behaviour(_ExportData(self.file_name, self.properties, self.interval, self.to_graph, self.graph_name, self.graph_options))

class _ExportDataSync(peak.CyclicBehaviour):

    def __init__(self, file_name: str, properties: list, to_graph: bool, graph_name: str, graph_options: str):
        super().__init__()
        self.file_name = file_name
        self.graph_name = graph_name
        self.properties = properties
...
class _ExportData(peak.PeriodicBehaviour):
    ...
    def __init__(self, file_name: str, properties: list, interval: float, to_graph: bool, graph_name: str, graph_options: str):
    ...
    async def run(self) -> None:
        ...
        for property in self.file_data:
            ...
            if type(attribute) is peak.Property:
                ...
                self.file_data[property].append(attribute)
                current_data[property] = attribute
            self.logger.debug('exporting property "' + property+ '": ' + str(current_data[property]))
        if self.to_graph:
            msg = peak.Message(to=df_name(self.agent.jid.domain))
